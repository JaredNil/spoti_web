...
export const Volume: React.FC = () => {
	...
	const volumeClickHandle = (event: React.MouseEvent<HTMLDivElement>) => {
		...
		let newVolume = Math.floor((
			event.pageX - Number(volumeRef.current?.offsetLeft))/(PLAYER_VOLUME_WIDTH/100)
		)
	...
	const volumeMouseMoveHandle = (event: React.MouseEvent<HTMLDivElement>) => {
		if(isResize 
			&& (event.pageX - Number(volumeRef.current?.offsetLeft)) <= PLAYER_VOLUME_WIDTH
			&& (event.pageX - Number(volumeRef.current?.offsetLeft)) >= 0
		)
			dispatch(playerAction.setVolume(
				Math.floor((event.pageX - Number(volumeRef.current?.offsetLeft))/(PLAYER_VOLUME_WIDTH/100))
			))
	}
	...
	return (
		...
		>
			...
			>
				<div style={{width: getVolumeWidth()}} 
