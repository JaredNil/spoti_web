...
export const PlayerLine: React.FC = () => {
	...
	useEffect(() => {
		...
		setHourTimer(
			td.getHours() <= 9 ? `0${td.getHours()}` : `${td.getHours()}`
		)
		setMinTimer(
			td.getMinutes() <= 9 ? `0${td.getMinutes()}` : `${td.getMinutes()}`
		)
		setSecTimer(
	...
	const progressClickHandle = (event: React.MouseEvent<HTMLDivElement>) => {
		if (track) {
			...
			let newProgress = line
				? Math.floor(
						(event.pageX - Number(lineRef.current?.offsetLeft)) /
							(line / 100)
					)
				: 0
