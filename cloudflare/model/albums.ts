...
export const listAlbums: Handler = async (_req, env) => {
	const albums = await list<AlbumInterface>(env.ALBUMS_KV, 'album:')
	return json(
...
export const getAlbum: Handler = async (_req, env, _ctx, { id }) => {
	const album = await get<AlbumInterface>(env.ALBUMS_KV, `album:${id}`)
...
export const createAlbum: Handler = async (req, env) => {
	const body = await req.json<AlbumInterface>()
	if (!body?.id) return httpError('Missing album id', 400)
...
export const updateAlbum: Handler = async (req, env, _ctx, { id }) => {
	const patch = await req.json<Partial<AlbumInterface>>()
	const key = `album:${id}`
	const old = await get<AlbumInterface>(env.ALBUMS_KV, key)
